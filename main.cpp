...
string keys[] = { ("Capslock"), ("Alt"), ("LShift"), ("Fire"), "E", "F", "T", "V", "C", "X" };
                                ...
                                InsideCircle(ScreenCenterX, ScreenCenterY, AimFOV, vHead.x, vHead.y))
                            {
                                if (DistanceBetweenCross(vHead.x, vHead.y))
                                {
                                    ...
                                    if (menu.items[16].value == 1) {
                                        RCS(AimAngles, GetCameraCache().Rotation, Smooth);
                                    }
                                    else
                                    {
           ...
           // int ObjID = pEntity.ObjectID;
            ...
            if (menu.items[0].value != 2 ? (pEntity.Type == Player) : (pEntity.Type == Bot))
            {
                ...
                if (Health > 0.f && Health < 200.f)
                {
                 ...
                 //   sprintf(d_char, xorstr_("%s"), CharacterName.c_str());
...
void InitializeMenu()
{
    menu.Header(xorstr_("LeagueHell"));//ASSASSINS(realdev) ~ UNBRANDED(reseller)
    ...
    menu.AddItem(xorstr_("Aim Key"), keys, 10, menu.items[11].value);
